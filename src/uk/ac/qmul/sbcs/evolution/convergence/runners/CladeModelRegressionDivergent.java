...
public class CladeModelRegressionDivergent {
	...
	public static void main(String[] args) {
		...
		HashMap<Integer,Double> siteOmegasAll		   	= cmo.getAllSiteOmegas();
		HashMap<Integer,Double> siteOmegasDivergent = cmo.getDivergentSiteOmegas();
		...
		double[] x = new double[siteOmegasDivergent.size()];
		double[] y = new double[siteOmegasDivergent.size()];
		if(printSites){
...
//				y[i] = w;
		...
		Iterator<Integer> siteItr = siteOmegasDivergent.keySet().iterator();
		int index=0;
		while(siteItr.hasNext()){
			int key = siteItr.next();
			float dSSLS = SSLS[key][0] - SSLS[key][numTrees-1];
			x[index] = dSSLS;
			y[index] = siteOmegasDivergent.get(key);
			index++;
		}
		if(!printSites){
