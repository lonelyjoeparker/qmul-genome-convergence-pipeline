...
//		showGraph("norm-point ÆSSLS exp");
		...
		for(int which=0;which<10;which++){
			...
			try {
				criticalValsH0H1[which] = this.H0H1DifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
			...
			// H0 - H2
			try {
				criticalValsH0H2[which] = this.H0H2DifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
			...
			// H0 - H3
			try {
				criticalValsH0H3[which] = this.H0H3DifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
			...
			// Next spp vs raxml
			try {
				criticalValsH0Rax[which] = this.H0RaxDifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
