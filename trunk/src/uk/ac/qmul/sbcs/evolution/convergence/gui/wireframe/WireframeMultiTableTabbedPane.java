 ...
 *
 */
public class WireframeMultiTableTabbedPane extends JTabbedPane implements ActionListener {

	private MyAlignmentsTableModel alignmentsModel;
	private MyAnalysesTableModel analysesModel;
	private MyResultsTableModel resultsModel;
	...
	private class AddResultsButtonListener implements ActionListener{
		@Override
		public void actionPerformed(ActionEvent ev) {
			int returnVal = fc.showOpenDialog(text);
			File resultFile = fc.getSelectedFile();
			// do nothing for now
			resultsModel.addRow(resultFile);
	...
	private class RowListener implements ListSelectionListener {
		public void valueChanged(ListSelectionEvent event) {
			...
			Object[] a_row = alignmentsModel.data[viewModelRow];
			...
			a_row = alignmentsModel.data[tableModelRow];
			val = (Integer)a_row[4];
			 ...
			 * Try the histogram
			 */
			double[] compEntropies = new double[alignmentsModel.data.length];
			double minVal = 0; //maximum to set hist limit
			double maxVal = 0; //maximum to set hist limit
			for(int i=0;i<compEntropies.length;i++){
				compEntropies[i] = ((Integer) alignmentsModel.data[i][4]).doubleValue();
	...
	class MyAnalysesTableModel extends AbstractTableModel {
	...
	class MyAlignmentsTableModel extends AbstractTableModel {
	 ...
	 * 
	 */
	public WireframeMultiTableTabbedPane() {
		...
		resultsModel = new MyResultsTableModel(true);
 ...
 */
        ...
        initColumnSizesAnyModel(resultsTable.getModel(), resultsTable);
		...
		JComponent resultsPanel = makeTextPanel("Results view");
...
//		add(tabbedPane);
	 ...
	 * @param arg0
	 */
	public WireframeMultiTableTabbedPane(int arg0) {
		super(arg0);
	 ...
	 * @param arg1
	 */
	public WireframeMultiTableTabbedPane(int arg0, int arg1) {
		super(arg0, arg1);
		// TODO Auto-generated constructor stub
	}
	 ...
	 */
	private void initColumnSizesAnyModel(TableModel model, JTable table) {
		TableColumn column = null;
		Component comp = null;
		int headerWidth = 0;
		int cellWidth = 0;
	...
	public JMenuBar createMenuBar() {
		...
		menuItem = new JMenuItem("Add results files (DUMMY)...");
		menuItem.addActionListener(new AddResultsButtonListener());
		resultsMenu.add(menuItem);
	...
	private void initResultsColumnSizes(JTable table) {
		MyResultsTableModel model = (MyResultsTableModel)table.getModel();
		TableColumn column = null;
		Component comp = null;
		int headerWidth = 0;
		int cellWidth = 0;
		Object[] longValues = model.longNames;
		TableCellRenderer headerRenderer =
			table.getTableHeader().getDefaultRenderer();
	...
	public static void createAndShowGUI() {
		...
		//Create and set up the content pane.
		WireframeMultiTableTabbedPane newContentPane = new WireframeMultiTableTabbedPane();
