...
//		treeDeNovolnL = new DataSeries(aaDataSSLSlnLdeNovo,"aa lnL data - RAxML tree");
		...
		File aaSppSimTreeTwoAnalysisOutputFile = new File(workDir.getAbsolutePath()+"/aamlSppSimFitTreeTwo.out");
		parameters.put(AamlParameters.SEQFILE, "seqfile = "+this.workDir.getAbsolutePath()+"/mc.paml");
		parameters.put(AamlParameters.TREEFILE, "treefile = "+this.treeFileTwoPruned.getAbsolutePath());
		parameters.put(AamlParameters.OUTFILE, "outfile = "+aaSppSimTreeTwoAnalysisOutputFile.getAbsolutePath());
		parameters.put(AamlParameters.AARATEFILE, "aaRatefile = "+this.binariesLocation.getAbsolutePath()+"/dat/wag.dat");
		parameters.put(AamlParameters.FIX_ALPHA, "fix_alpha = 1");
		parameters.put(AamlParameters.ALPHA, "alpha = "+alphaSpp);
		treefiles[0] = this.treeFileTwoPruned;
		AamlAnalysisSGE treeTwoAamlSim = new AamlAnalysisSGE(datasets, treefiles, parameters,"aamlSppSimOnTreeTwo.ctl");
		...
		treeOneSimlnLOnTreeTwo = new ExperimentalDataSeries(simulatedSpp.getFullSitesLnL(aaDataTreeTwoSimSSLS));
		...
		treeDeNovoAamlSim.setExecutionBinary(new File(treeTwoAamlSim.getBinaryDir(),"codeml"));
...
//		treeDeNovoSimlnL = new DataSeries(aaDataSimSSLSlnLrax,"aa lnL data - tree de novo (sim)");
		...
		*/
		...
		try {
			for(int i=0;i<101;i++){
				treeTwoSimPercentiles[i] = treeOneSimlnLOnTreeTwo.getValueAtPercentile(intervals[i]);
			}
		} catch (PercentileOutOfRangeError e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		for(int i=0;i<101;i++){
			try {
				speciesVsPrestinDSSLSobs[i] = sppPreDifferencesObs.getValueAtPercentile(intervals[i]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		for(int i=0;i<101;i++){
			try {
				speciesVsDeNovoDSSLSobs[i] = sppRaxDifferencesObs.getValueAtPercentile(intervals[i]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		for(int i=0;i<101;i++){
			try {
				speciesVsPrestinDSSLSexp[i] = sppPreDifferencesExp.getValueAtPercentile(intervals[i]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		for(int i=0;i<101;i++){
			try {
				speciesVsDeNovoDSSLSexp[i] = sppRaxDifferencesExp.getValueAtPercentile(intervals[i]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		logfileData.append("Sitewise dSSLS (obs) for:\tSpp-Pre\tSpp-Rax\n");
		...
		spDf = sppPreDifferencesExp.getData();
		srDf = sppRaxDifferencesExp.getData();
	//	for(int i=0;i<spDf.length;i++){
	//		logfileData.append(i+"\t"+spDf[i]+"\t\t"+srDf[i]+"\n");
	//	}
...
//		dCDF = sppPreDifferencesExp.getCountsPDFCDFDataDeprecateMeEfficient(explicitMin, explicitMax);
		...
		float[] criticalValsSppPre = new float[10];
		float[] criticalValsSppRax = new float[10];
		int[] extremeCountsSppPre = new int[10];
		int[] extremeCountsSppRax = new int[10];
		
		for(int which=0;which<10;which++){
			// First spp vs prestin
			try {
				criticalValsSppPre[which] = this.sppPreDifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
				extremeCountsSppPre[which] = this.sppPreDifferencesObs.getNumberLessThan(criticalValsSppPre[which]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				criticalValsSppPre[which] = Float.NaN;
				extremeCountsSppPre[which] = 0;
				e.printStackTrace();
			}
			// Next spp vs raxml
			try {
				criticalValsSppRax[which] = this.sppRaxDifferencesExp.getThresholdValueAtCumulativeDensity(densities[which]);
				extremeCountsSppRax[which] = this.sppPreDifferencesObs.getNumberLessThan(criticalValsSppRax[which]);
			} catch (PercentileOutOfRangeError e) {
				// TODO Auto-generated catch block
				criticalValsSppRax[which] = Float.NaN;
				extremeCountsSppRax[which] = 0;
				e.printStackTrace();
			}
			logfileData.append(which+"\t"+criticalValsSppPre[which]+"\t"+extremeCountsSppPre[which]+"\t"+criticalValsSppRax[which]+"\t"+extremeCountsSppRax[which]+"\n");
		}
		long elapsed = System.currentTimeMillis() - time;
		logfileData.append("Total time "+elapsed+"s.");
