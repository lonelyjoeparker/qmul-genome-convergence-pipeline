 ...
 */
public class CongruenceAnalysisAAWithBinariesPruningSimulationFilteringSimCDFs {
	...
	private ExperimentalDataSeries treeTwoSimOnTreeOnelnL;
	...
	public CongruenceAnalysisAAWithBinariesPruningSimulationFilteringSimCDFs(File data, File one, File two, File work, File binariesLocation, String ID, TreeSet<String> taxaList, int sitesToSimulate, int thisFilter, boolean filterThisByFactor){
		this.dataset = data;
	...
	//	ra.setWorkingDir(this.workDir);
		...
		AlignedSequenceRepresentation[] datasets = {new AlignedSequenceRepresentation()};
				// NOTE: We are assuming the *actual* datafile is present in AamlParameters.SEQFILE..
		...
		float[] aaDataSSLSlnL1 = new float[aaDataTreeOneSSLS.size()];				// TODO this is quite a laborious way to init and populate the lnL..  (27/3/2012)
...
//		treeTwoSimlnL = new DataSeries(aaDataSimSSLSlnL2,"aa lnL data - tree 2 (sim)");
		...
		/**
		 * @since version 1 of this class (equiv version 86 of CongruenceAnalysisAAWithBinariesPruningSimulationFilteringCDFsSandbox)
		 * Simulated data from TREE 2
		 * Paml carried out on TREE 1
		 * Use this lnL for the H0 distribution of sites... 
		 * 
		 *  Do Aaml on data simulated on prestin tree */

		File aaSimTreeTwoOnTreeOneAnalysisOutputFile = new File(workDir.getAbsolutePath()+"/aamlSimTreeTwo.out");
		parameters.put(AamlParameters.SEQFILE, "seqfile = "+this.workDir.getAbsolutePath()+"/mc.paml");
		parameters.put(AamlParameters.TREEFILE, "treefile = "+this.treeFileTwoPruned.getAbsolutePath());
		parameters.put(AamlParameters.OUTFILE, "outfile = "+aaSimTreeTwoOnTreeOneAnalysisOutputFile.getAbsolutePath());
		parameters.put(AamlParameters.AARATEFILE, "aaRatefile = "+this.binariesLocation.getAbsolutePath()+"/dat/wag.dat");
		parameters.put(AamlParameters.FIX_ALPHA, "fix_alpha = 1");
		parameters.put(AamlParameters.ALPHA, "alpha = "+ alphaPre);
		treefiles[0] = this.treeFileTwoPruned;
		datasets[0] = simulatedPre;
		AamlAnalysisSGE treeTwoOnTreeOneAamlSim = new AamlAnalysisSGE(datasets, treefiles, parameters,"aamlOnTreePreSim.ctl");
		treeTwoOnTreeOneAamlSim.setBinaryDir(this.binariesLocation.getAbsoluteFile());
		treeTwoOnTreeOneAamlSim.setExecutionBinary(new File(treeTwoAamlSim.getBinaryDir(),"codeml"));
		treeTwoOnTreeOneAamlSim.setWorkingDir(workDir);
		treeTwoOnTreeOneAamlSim.RunAnalysis();
		TreeMap<String, Float> aaDataTreeTwoOnTreeOneSimSSLS = treeTwoOnTreeOneAamlSim.getPatternSSLS();
		float[] aaDataSimSSLSlnL2on1 = new float[aaDataTreeTwoOnTreeOneSimSSLS.size()];
		Iterator dataSimSSLSItr2on1 = aaDataTreeTwoOnTreeOneSimSSLS.keySet().iterator();
		sIndex = 0;
		while(dataSimSSLSItr2on1.hasNext()){
			aaDataSimSSLSlnL2on1[sIndex] = aaDataTreeTwoOnTreeOneSimSSLS.get(dataSimSSLSItr2on1.next());
			sIndex++;
		}
//		treeTwoSimlnL = new DataSeries(aaDataSimSSLSlnL2,"aa lnL data - tree 2 (sim)");
		treeTwoSimOnTreeOnelnL = new ExperimentalDataSeries(simulatedPre.getFullSitesLnL(aaDataTreeTwoOnTreeOneSimSSLS));
